...
- [x] `python -m finger_tracker.detection` でリアルタイム指間距離計測が動作する
- [x] RGB 映像 + BB オーバーレイ + 距離表示のシングルウィンドウが表示される
- [x] カルマンフィルタにより距離値が安定している
- [x] 深度欠損・未検出時に ADR 002 の対処が正しく機能する
- [x] `logs/` に CSV 計測記録が保存される
- [x] `q` / ESC で安全に終了し、CSV がフラッシュされる
