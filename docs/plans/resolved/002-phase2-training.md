...
- [ ] `python -m finger_tracker.training` で YOLOv8-nano の fine-tuning が実行できる
- [ ] 学習完了後に mAP50 等の評価指標がコンソールに表示される
- [ ] `models/best.pt` に学習済みモデルが配置される
- [ ] mAP50 >= 0.90 を目標とし、未達時の対応方針が明確
